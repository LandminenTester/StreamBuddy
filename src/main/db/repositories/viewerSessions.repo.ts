...
export function closeViewerSession(
  streamId: string,
  userLogin: string,
  leftAt: number
): void {
...
export function closeAllOpenSessions(streamId: string, leftAt: number): void {
  getDb()
    .prepare(
      'UPDATE viewer_sessions SET left_at = ? WHERE stream_id = ? AND left_at IS NULL'
    )
    .run(leftAt, streamId)
}
...
export function getViewerSessionsByStream(streamId: string): ViewerSession[] {
  return getDb()
    .prepare<[string], SessionRow>(
      'SELECT * FROM viewer_sessions WHERE stream_id = ? ORDER BY joined_at ASC'
    )
    .all(streamId)
    .map((row) => ({
      id: row.id,
...
export function getStreamStats(streamId: string): StreamStats {
  const rows = getDb()
    .prepare<
      [string],
      { user_login: string; total_seconds: number }
    >(
      `SELECT user_login,
