...
const secondMarks = computed(() => {
  const count = Math.floor(props.rangeMs / 1000)
...
function onPointerMove(event: PointerEvent): void {
  ...
  switch (drag.value.mode) {
    ...
    case 'resize-end':
      next.durationMs = clamp(
        original.durationMs + snapped,
        props.minDurationMs,
...
<template>
  <div class="space-y-1">
    ...
    <div ref="trackEl" class="relative h-10 rounded border border-line bg-surface-subtle">
      <span
        v-for="mark in secondMarks"
        :key="mark"
        class="pointer-events-none absolute top-0 h-full border-l border-line/50 text-[10px] text-fg-subtle"
        :style="{ left: `${(mark * 1000 / rangeMs) * 100}%` }"
      >
        <span class="ml-0.5">{{ mark }}s</span>
      </span>

      <div
        class="absolute top-1 h-8 cursor-grab rounded bg-accent/70 active:cursor-grabbing"
