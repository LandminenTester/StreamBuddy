...
const previewEffect = ref<Effect | null>(null)
...
function emptyForm(): EffectFormState {
  return { id: null, name: '', videoPath: null, audioPath: null, width: 1920, height: 1080 }
}
...
async function confirmDelete(): Promise<void> {
  ...
  if (previewEffect.value?.id === deleteTarget.value.id) previewEffect.value = null
...
const previewVideoUrl = computed(() =>
  previewEffect.value && store.serverPort
    ? `http://localhost:${store.serverPort}/overlay/media/video/${previewEffect.value.id}`
    : null
)
</script>
...
<template>
  <div class="mx-auto max-w-4xl space-y-8">
    ...
    <PageSection v-if="previewEffect" :title="$t('alerts.form.preview')">
      <div class="flex flex-col gap-3">
        <p class="text-sm text-fg-muted">{{ previewEffect.name }}</p>
        <video
          v-if="previewVideoUrl"
          :key="previewVideoUrl"
          :src="previewVideoUrl"
          controls
          class="max-w-sm rounded-md border border-line"
          style="max-height: 240px"
        />
        <p v-else class="text-sm text-fg-subtle">{{ $t('alerts.form.noVideoFile') }}</p>
      </div>
    </PageSection>
  </div>
